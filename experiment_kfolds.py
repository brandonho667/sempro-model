...
class Experiment(object):
    ...
    def __init__(self, name="default", root_dir="./", stats_dir="experiments", linear=False):
        try:
    ...
    def init_model(self, fold, dataset, train_idx, test_idx):
        # Initialize experiment folder directory
        self.__name = f"{self.config_data['experiment_name']}_fold_{fold}"
        print(f"Loading {self.__name}")
        self.__expt_dir = os.path.join(
            self.root_dir, self.stats_dir, self.__name)
        os.makedirs(self.__expt_dir, exist_ok=True)
        self.__train_log_path = os.path.join(
            self.__expt_dir, self.config_data['output']["train_file_suffix"])
        self.__plot_folder_path = os.path.join(
            self.__expt_dir, self.config_data['output']["plot_folder_suffix"])
        os.makedirs(self.__plot_folder_path, exist_ok=True)
        self.__best_model_path = os.path.join(
            self.__expt_dir, self.config_data['output']["best_model_path"])
        self.dpi = self.config_data['output']['dpi']

        # Transforms go here
        # TODO: Modularize rescaling factors

        # Configurable transforms
        self.rotate = self.config_data['transforms']['rotate']
        self.resize = self.config_data['transforms']['resize']
        self.crop = self.config_data['transforms']['crop']
        self.normalize = self.config_data['transforms']['normalize']
        ...
        train_idx, val_idx = train_test_split(train_idx,
                                              test_size=self.config_data['dataset']["val_split"],
                                              random_state=self.config_data['dataset']["random_state"])
        self.train_data = torch.utils.data.Subset(
            dataset(filepath=self.config_data['dataset']['filepath'], transform=transform, lds=self.config_data['hparams']['lds'], lds_ks=self.config_data['hparams']['lds_ks'], lds_sigma=self.config_data['hparams']['lds_sigma'], bf=self.config_data['hparams']['bf']), train_idx)
        self.train_eval_data = torch.utils.data.Subset(
            dataset(filepath=self.config_data['dataset']['filepath'], transform=eval_transform, lds=self.config_data['hparams']['lds'], lds_ks=self.config_data['hparams']['lds_ks'], lds_sigma=self.config_data['hparams']['lds_sigma'], bf=self.config_data['hparams']['bf']), train_idx)
        self.val_data = torch.utils.data.Subset(
            dataset(filepath=self.config_data['dataset']['filepath'], transform=eval_transform, lds=self.config_data['hparams']['lds'], lds_ks=self.config_data['hparams']['lds_ks'], lds_sigma=self.config_data['hparams']['lds_sigma'], bf=self.config_data['hparams']['bf']), val_idx)
        self.test_data = torch.utils.data.Subset(
            dataset(filepath=self.config_data['dataset']['filepath'], transform=eval_transform, lds=self.config_data['hparams']['lds'], lds_ks=self.config_data['hparams']['lds_ks'], lds_sigma=self.config_data['hparams']['lds_sigma'], bf=self.config_data['hparams']['bf']), test_idx)
        if self.config_data['experiment_name'] == "test":
            self.test_data = dataset(filepath=self.config_data['dataset']['filepath'],
                                     transform=eval_transform, lds=self.config_data['hparams']['lds'], lds_ks=self.config_data['hparams']['lds_ks'], lds_sigma=self.config_data['hparams']['lds_sigma'], bf=self.config_data['hparams']['bf'])

        self.batch_size = self.config_data['hparams']['batch_size']
        ...
        # Intialize model
        pretrained = self.config_data["model"]['pretrained_model']
        # If you want to add a potential model, modify this
        if self.config_data["model"]["model_name"] == "resnext_large":
            self.model = SEMPro_resNext(fc_size=self.config_data["model"]['fc_size'],
                                        large=True, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "resnext_small":
            self.model = SEMPro_resNext(fc_size=self.config_data["model"]['fc_size'],
                                        large=False, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "densenet121":
            self.model = SEMPro_denseNet(fc_size=self.config_data["model"]['fc_size'],
                                         size=0, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "densenet169":
            self.model = SEMPro_denseNet(fc_size=self.config_data["model"]['fc_size'],
                                         size=1, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "densenet201":
            self.model = SEMPro_denseNet(fc_size=self.config_data["model"]['fc_size'],
                                         size=2, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "densenet161":
            self.model = SEMPro_denseNet(fc_size=self.config_data["model"]['fc_size'],
                                         size=3, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "convnext_tiny":
            self.model = SEMPro_ConvNext(fc_size=self.config_data["model"]['fc_size'],
                                         size=0, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "convnext_small":
            self.model = SEMPro_ConvNext(fc_size=self.config_data["model"]['fc_size'],
                                         size=1, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "convnext_base":
            self.model = SEMPro_ConvNext(fc_size=self.config_data["model"]['fc_size'],
                                         size=2, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "convnext_large":
            self.model = SEMPro_ConvNext(fc_size=self.config_data["model"]['fc_size'],
                                         size=3, pretrained=pretrained)
        elif self.config_data["model"]["model_name"] == "baseplate":
        ...
        print("Model initialized w/ following trainable parameters:")
        ...
        if self.config_data['hparams']['optimizer'] == "AdamW":
            self.optimizer = torch.optim.AdamW(self.model.parameters(),
                                               lr=self.config_data['hparams']["lr"],
                                               weight_decay=self.config_data['hparams']["weight_decay"])
        elif self.config_data['hparams']['optimizer'] == "Adam":
        ...
        if self.config_data['model']['criterion'] == 'L1':
            self.criterion = nn.L1Loss()
        elif self.config_data['model']['criterion'] == 'L2':
        ...
        if self.config_data['hparams']['LR_scheduler'] == 'step':
            self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, self.config_data['hparams']['steps_per_decay'],
                                                             gamma=self.config_data['hparams']['LR_gamma'])
        elif self.config_data['hparams']['LR_scheduler'] == 'plateau':
    ...
    def kfolds(self):
        ...
        if self.config_data['dataset']["sem_dataset"] == "sem_dataset":
            dataset = semDataset
        else:
            raise Exception("Dataset not supported!")
        kfold = KFold(
            n_splits=int(1/self.config_data['dataset']["test_split"]), shuffle=True, random_state=42)
        fold_perf = {}
        for fold, (train_ids, test_ids) in enumerate(kfold.split(range(len(dataset())))):
            print(f'FOLD {fold}')
            print('--------------------------------')
            self.init_model(fold, dataset, train_ids, test_ids)
            self.train(fold)
            mae, mse = self.test(fold)
            fold_perf[f"fold_{fold}"] = {"mae": mae, "mse": mse}
            self.analyze_training()
            self.analyze_error_dist()
            del self.model
        ave_mae = np.mean([fold_perf[f]['mae'] for f in fold_perf])
        ave_mse = np.mean([fold_perf[f]['mse'] for f in fold_perf])
        fold_perf['ave_mae'] = ave_mae
        fold_perf['ave_mse'] = ave_mse
        json.dump(fold_perf, open(
            f"experiments/{self.__name}_fold_perf.json", "w"))

    def kfold_test_spec(self, fold_model, folds):
        if self.config_data['dataset']["sem_dataset"] == "sem_dataset":
            dataset = semDataset
        else:
            raise Exception("Dataset not supported!")
        ...
        for fold, (train_ids, test_ids) in enumerate(kfold.split(range(len(dataset())))):
            ...
            print('--------------------------------')
            self.init_model(fold, dataset, train_ids, test_ids)
            name = f"{fold_model}_fold_{fold}"
            path = os.path.join(self.root_dir, self.stats_dir, name, "best.pt")
            self.set_best_model(path)
            mae, mse = self.test(fold, raw=False)
            maes.append(mae)
            mses.append(mse)
        return maes, mses

    def kfold_test(self, fold_model, folds):
        # Initialize dataset, for this case there should only be one dataset
        if self.config_data['dataset']["sem_dataset"] == "sem_dataset":
            dataset = semDataset
        else:
        ...
        self.test_data = dataset(filepath=self.config_data['dataset']['filepath'],
                                 transform=eval_transform, lds_ks=self.config_data['hparams']['lds_ks'], lds_sigma=self.config_data['hparams']['lds_sigma'], bf=self.config_data['hparams']['bf'])
    ...
    def train(self, fold):
        ...
        for epoch in range(self.epochs):
            print(f"Now running epoch {epoch} of fold {fold}")
    ...
    def test(self, fold, raw=False):
        ...
        print(
            f"FOLD {fold} Test loss: MAE = {test_mae_loss}, MSE = {test_mse_loss}")
        del model
